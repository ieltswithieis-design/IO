...
async function supabaseRegister(name, email, phone, qualification, password) {
...
function topbar(){
  const u=user();
  return `<header class="topbar"><div class="brand">${logo()}<span>IELTS Academic Reading Platform</span></div>
  <nav class="nav">
...
async function login(){
  ...
  try{
    ...
    const u=data.user;
    setSession({id:u.id,name:u.user_metadata?.full_name||email.split("@")[0],email:u.email||email,role:u.user_metadata?.role||"student",country:u.user_metadata?.country||"",createdAt:u.created_at});
  ...
  app.innerHTML=`<div class="auth-wrap"><div class="auth-shell">
    ...
    <section class="auth-form-panel">
      ...
      <form onsubmit="event.preventDefault();signup()">
        ...
        <div class="field"><label>Phone number</label><input id="phone" type="text" autocomplete="tel" placeholder="Your phone number" required></div>
        <div class="field"><label>Last qualification</label><input id="qualification" type="text" autocomplete="organization-title" placeholder="e.g. Intermediate, Bachelor's, Master's" required></div>
...
async function signup(){
  ...
  const msg=$("#msg");
  if(!name||!email){msg.innerHTML='<div class="notice error">Please enter your full name and email address.</div>';return}
  if(!phone){msg.innerHTML='<div class="notice error">Please enter your phone number.</div>';return}
  if(!qualification){msg.innerHTML='<div class="notice error">Please enter your last qualification.</div>';return}
  ...
  try{
    const data=await supabaseRegister(name,email,phone,qualification,p);
    ...
    const u=data.user;
    setSession({id:u.id,name:u.user_metadata?.full_name||name,email:u.email||email,role:u.user_metadata?.role||"student",phone:u.user_metadata?.phone_number||phone,qualification:u.user_metadata?.last_qualification||qualification,country:$("#country").value.trim(),createdAt:u.created_at});
    route("dashboard");
  }catch(e){
...
async function restoreSupabaseSession(){
  try{
    ...
    if(data?.session?.user){
      ...
      setSession({id:u.id,name:u.user_metadata?.full_name||u.email?.split("@")[0]||"Student",email:u.email||"",role:u.user_metadata?.role||"student",country:u.user_metadata?.country||"",createdAt:u.created_at});
    }else{
      setSession(null);
    }
  }catch(e){
